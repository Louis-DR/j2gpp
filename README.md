...
Representation of integers in different bases are available with the filters `bin`, `hex`, and `oct` for binary, hexadecimal, and octal respectively. An optional length can be provided and the output will be padded with zeros. No prefix such as `0b` or `0x` are added.
