...
Restructuring of large blocks is facilitated by the filter `restructure` that should be used on a `filter` block. The block will be parsed for structure tags, which is an addition to the Jinja2 syntax and uses the `{§...§}` syntax. Structure tags currently implemented are `spacing` and `align`. The `spacing` tag (`{§spacing [lines]§}`) takes a single parameter representing a number of lines. When the restructuring filter processes the tag, it will strip all line breaks before and after the tag and replace them with the number of line breaks specified. This is useful to add spacing to the template for readability while keeping the spacing of the generated file clean. The `align` tag (`{§align [left|right|center] [index]§}`) acts as a more controllable alignment marker compared to the `align` filter. It allows you to explicitly configure the justification direction and an optional numerical column index. This makes it possible to skip column alignments on certain lines or structure spanning column layouts accurately.
